...
def launch_worker(input_files: FilesystemBinary, output_dir: str, operation: str = "image"):
    tmpdir = mktemp()
    ...
    for subdir in glob.glob(f"{tmpdir}/*"):
        ...
        reducer_result, reduce_time = meassure_time(lambda: __launch_convertmc(subdir, output_dir, operation))
...
def __launch_convertmc(input_dir: str, output_dir: str, operation):
    cmd(
        f"binaries/convertmc {operation} --many {input_dir}/* {output_dir}"
    )
