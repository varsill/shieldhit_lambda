...
def mapper(n, N, files, should_produce_hdf, save_to):
    ...
    elif (type(save_to) is tuple or type(save_to) is list) and len(save_to)==2 and save_to[0]=="volume":
...
def reducer(files, get_from, operation, worker_id_prefix):
    ...
    if get_from == "uploaded":
        ...
        for just_filename in files.keys():
            client.download_file(BUCKET, files[just_filename], f"{tmpdir}/{just_filename}")
