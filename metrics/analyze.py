...
RESULTS_DUMP_FILE = "metrics/results/test_results.dump"
...
def plot(results):
    ...
    results = results.drop(
        columns=["results.workers_times", "params.number_of_samples", "test_run_number"]
    )
    # to_plot = results.filter(axis="columns", items=["test_run_number", "params.number_of_workers"])
    # print(results.iloc[0])
    results = results.groupby("params.number_of_workers").mean(
        ["results.map_time", "results.reduce_time", "results.total_duration"]
    )

    plt.plot(results["results.map_time"], label="Map time")
