...
def plot_simulation_time_to_request_time_vs_param(
    results, group_by_param, plot_filename, workers, title
):
    plt.tight_layout(pad=2)
    results["mean_simulation_time"] = results.apply(
        lambda row: sum(row[f"metrics.{workers}_simulation_times"])
        / len(row[f"metrics.{workers}_simulation_times"])
        if len(row[f"metrics.{workers}_simulation_times"]) > 0
        else 0,
        axis=1,
    )
    results["mean_request_time"] = results.apply(
        lambda row: sum(row[f"metrics.{workers}_request_times"])
        / len(row[f"metrics.{workers}_request_times"])
        if len(row[f"metrics.{workers}_request_times"]) > 0
        else 0,
        axis=1,
    )
    results["simulation_to_request"] = results.apply(
        lambda row: row["mean_simulation_time"] / row["mean_request_time"]
    ...
    plt.bar(
        results_avg[f"params.{group_by_param}"],
        results_avg["mean_simulation_time"],
        5,
...
def plot_percentage_of_successfull_responses(
    results, group_by_param, plot_filename, title
):
    results["successfull_responses_perc"] = results.apply(
        lambda row: len(row["metrics.mappers_request_times"])
        / row["params.number_of_workers"],
        axis=1,
    )
    results_avg = (
        results.groupby(f"params.{group_by_param}")
...
def plot_map_reduce_total_time_vs_params(
    results, group_by_param, plot_filename, times_fields_list, title
):
    results_avg = (
        results.groupby(f"params.{group_by_param}")
...
def plot_simulation_time_and_request_time_ratio_vs_param(
    results, group_by_param, plot_filename, workers, title
):
    results["mean_simulation_time"] = results.apply(
        lambda row: sum(row[f"metrics.{workers}_simulation_times"])
    ...
    results["simulation_to_request"] = results.apply(
        lambda row: row["mean_simulation_time"] / row["mean_request_time"] * 100
        if row["mean_request_time"] > 0
        else 0,
        axis=1,
    )
    ...
    plt.errorbar(
        results_avg[f"params.{group_by_param}"], results_avg["simulation_to_request"]
    )
    plt.legend()
    plt.xlabel(f"{group_by_param}")
    plt.ylabel(f"[%]")
...
def plot_cumulative_time_vs_params(
    ...
    results["simulation_time_sum"] = results.apply(
        lambda row: sum(row[f"metrics.{workers}_simulation_times"]), axis=1
    )
    results["request_time_sum"] = results.apply(
        lambda row: sum(row[f"metrics.{workers}_request_times"]), axis=1
    )
    results_avg = (
        results.groupby(f"params.{group_by_param}")
